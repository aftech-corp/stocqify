...
$stmt = $db->prepare('SELECT * FROM branches WHERE id=? AND business_id=?');
$stmt->execute([$id, $bizId]);
$branch = $stmt->fetch();
if (!$branch) { flash('error', 'Branch not found.'); redirect(url('branches')); }
...
if (isPost()) {
    ...
    if ($code) {
        $codeChk = $db->prepare('SELECT id FROM branches WHERE business_id=? AND code=? AND id!=?');
        $codeChk->execute([$bizId, $code, $id]);
    ...
    if (empty($errors)) {
        $db->prepare('UPDATE branches SET name=?,code=?,address=?,phone=?,email=?,manager_user_id=?,manager_name=?,notes=?,is_active=?,updated_at=NOW() WHERE id=? AND business_id=?')
           ->execute([$name, $code ?: null, $address ?: null, $phone ?: null, $email ?: null, $managerUserId, $managerName, $notes ?: null, $isActive, $id, $bizId]);

        // Clear session if this was the active branch and it got deactivated
        if (!$isActive && currentBranchId() === $id) {
    ...
    $_POST = $branch;
    $_POST['is_active']       = (string)$branch['is_active'];
    $_POST['manager_user_id'] = (string)($branch['manager_user_id'] ?? '');
}
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="manager_user_id" class="w-full border border-gray-300 rounded-lg text-sm px-3 py-2 outline-none focus:ring-2 focus:ring-blue-500">
                        ...
                        <option value="<?= $u['id'] ?>" <?= (string)(int)post('manager_user_id') === (string)$u['id'] ? 'selected' : '' ?>>
                            <?= h($u['name']) ?> (<?= h($u['role_name']) ?>)
                        </option>
                        <?php endforeach; ?>
                    </select>
                    <?php if (empty($bizUsers)): ?>
                    <p class="text-xs text-amber-600 mt-0.5"><i class="fa-solid fa-circle-info mr-1"></i>No other users registered yet.</p>
                    <?php endif; ?>
                </div>
                <div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="address" rows="2"
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none resize-none"><?= h(post('address')) ?></textarea>
                </div>
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Notes</label>
                    <textarea name="notes" rows="2"
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none resize-none"><?= h(post('notes')) ?></textarea>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Status</label>
                    <select name="is_active" class="w-full border border-gray-300 rounded-lg text-sm px-3 py-2 outline-none">
                        <option value="1" <?= post('is_active') === '1' ? 'selected' : '' ?>>Active</option>
                        <option value="0" <?= post('is_active') === '0' ? 'selected' : '' ?>>Inactive</option>
                    </select>
                    <p class="text-xs text-gray-400 mt-0.5">Inactive branches are hidden from the branch switcher.</p>
                </div>
            </div>
            <div class="flex gap-3 mt-6">
                <button type="submit" class="bg-blue-600 text-white px-6 py-2 rounded-lg text-sm font-medium hover:bg-blue-700">
                    <i class="fa-solid fa-save mr-1"></i> Update Branch
                </button>
