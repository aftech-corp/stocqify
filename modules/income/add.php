...
if (isPost()) {
    ...
    if (empty($errors)) {
        $stmt = $db->prepare('INSERT INTO income (business_id,user_id,title,description,amount,payment_method,reference_number,income_date,branch_id) VALUES (?,?,?,?,?,?,?,?,?)');
        $stmt->execute([$bizId, currentUser()['id'], $title, $desc?:null, $amount, $method, $ref?:null, $date, $branchId?:null]);
